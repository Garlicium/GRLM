...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 3840
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 3839
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 13840
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 13839
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 3840
	%{_sbindir}/semanage port -d -p tcp 3839
	%{_sbindir}/semanage port -d -p tcp 13840
	%{_sbindir}/semanage port -d -p tcp 13839
	for selinuxvariant in %{selinux_variants}; do
